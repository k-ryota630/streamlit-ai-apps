...
try:
    ...
    st.error("`langchain_anthropic` が見つかりません。インストールしてください。")
    st.stop()
...
def main():
    st.set_page_config(page_title="My Great Claude 4 opus", page_icon="🤗")
    st.header("My Great Claude 4 opus 🤗")
    ...
    if user_input:
        ...
        with st.spinner("Claude4 opus is thinking..."):
            try:
                langchain_messages = []
